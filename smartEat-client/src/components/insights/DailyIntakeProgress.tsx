import { Cookie, Dumbbell, Flame, Wheat } from "lucide-react";
import React from "react";
import { DailyNutrition, DailyRecommendations } from "../../hooks/nutrition/useDailyNutrition";
...
const DailyIntakeProgress: React.FC<DailyIntakeProgressProps> = ({ 
  ...
  const nutrients: NutrientProgress[] = currentNutrition && recommendations ? [
    {
              ...
              <div className="mt-1 flex justify-between items-center">
                <span className="text-xs font-medium" style={{ color: progress > 100 ? '#ef4444' : '#6b7280' }}>
                  {progress > 100 ? 'Exceeded' : `${Math.round(progress)}%`}
                   ...
                   {formatValue(Math.round(Math.max(0, nutrient.target - nutrient.current)), nutrient.label)} remaining
                 </span>
              </div>
            </div>
          );
