...
export interface GetDailyExerciseGoalResponse {
  calories: number;
}
...
export const GetDailyExerciseGoalResponse: MessageFns<GetDailyExerciseGoalResponse> = {
  encode(message: GetDailyExerciseGoalResponse, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.calories !== 0) {
      writer.uint32(9).double(message.calories);
    }
    return writer;
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): GetDailyExerciseGoalResponse {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
...
export const DailyOpinionRequest: MessageFns<DailyOpinionRequest> = {
  encode(message: DailyOpinionRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.user !== undefined) {
      UserProfile.encode(message.user, writer.uint32(10).fork()).join();
    }
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): DailyOpinionRequest {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseDailyOpinionRequest();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
...
function createBaseDailyOpinionResponse(): DailyOpinionResponse {
  return { opinion: "", recommendations: [], positiveFeedback: "" };
}

export const DailyOpinionResponse: MessageFns<DailyOpinionResponse> = {
  encode(message: DailyOpinionResponse, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.opinion !== "") {
      writer.uint32(10).string(message.opinion);
    }
    for (const v of message.recommendations) {
      writer.uint32(18).string(v!);
    }
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): DailyOpinionResponse {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseDailyOpinionResponse();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
...
export interface NutritionsRatingServiceClient {
  ...
  getDailyExerciseGoal(request: UserProfile): Observable<GetDailyExerciseGoalResponse>;

  getDailyOpinion(request: DailyOpinionRequest): Observable<DailyOpinionResponse>;
}
...
export interface NutritionsRatingServiceController {
  ...
  getDailyExerciseGoal(
    request: UserProfile,
  ): Promise<GetDailyExerciseGoalResponse> | Observable<GetDailyExerciseGoalResponse> | GetDailyExerciseGoalResponse;

  getDailyOpinion(
    request: DailyOpinionRequest,
  ): Promise<DailyOpinionResponse> | Observable<DailyOpinionResponse> | DailyOpinionResponse;
}

export function NutritionsRatingServiceControllerMethods() {
  return function (constructor: Function) {
    const grpcMethods: string[] = ["analyzeMeal", "getDailyRecommendations", "getDailyExerciseGoal", "getDailyOpinion"];
...
export const NutritionsRatingServiceService = {
  ...
  getDailyExerciseGoal: {
    ...
    responseDeserialize: (value: Buffer) => GetDailyExerciseGoalResponse.decode(value),
  },
  getDailyOpinion: {
    path: "/nutrition.NutritionsRatingService/GetDailyOpinion",
    requestStream: false,
    responseStream: false,
    requestSerialize: (value: DailyOpinionRequest) => Buffer.from(DailyOpinionRequest.encode(value).finish()),
    requestDeserialize: (value: Buffer) => DailyOpinionRequest.decode(value),
    responseSerialize: (value: DailyOpinionResponse) => Buffer.from(DailyOpinionResponse.encode(value).finish()),
    responseDeserialize: (value: Buffer) => DailyOpinionResponse.decode(value),
  },
} as const;
...
export interface NutritionsRatingServiceServer extends UntypedServiceImplementation {
  ...
  getDailyExerciseGoal: handleUnaryCall<UserProfile, GetDailyExerciseGoalResponse>;
  getDailyOpinion: handleUnaryCall<DailyOpinionRequest, DailyOpinionResponse>;
}
